...
export class AddEventLocationComponent implements OnInit {
  ...
  filteredAdminLevelOnes: ReplaySubject<AdministrativeLevelOne[]> =
    new ReplaySubject<AdministrativeLevelOne[]>();
  filteredAdminLevelTwos: ReplaySubject<AdministrativeLevelTwo[]> =
  ...
  onSubmit(formValue) {
    ...
    formValue.start_date = this.datePipe.transform(
      formValue.start_date,
      "yyyy-MM-dd"
    );
    formValue.end_date = this.datePipe.transform(
      formValue.end_date,
      "yyyy-MM-dd"
    );
    ...
    if (this.addEventLocationForm.get("new_location_contacts") != null) {
      const control = <FormArray>(
        this.addEventLocationForm.get("new_location_contacts")
      );
      this.addEventLocationForm
        .get("new_location_contacts")
    ...
    this.eventLocationService.create(formValue).subscribe(
      (newEventLocation) => {
        this.submitLoading = false;
        this.openSnackBar(
          "New event location successfully created. Page will reload.",
          "OK",
          5000
        );
        ...
        gtag("event", "click", {
          ...
          event_label: "New Location Added",
        });
      },
      (error) => {
        this.errorMessage = <any>error;
        this.submitLoading = false;
        this.openSnackBar(
          "Error. Event location not created. Error message: " + error,
          "OK",
          8000
        );
      }
  ...
  checkforMissingSpecies() {
    ...
    const locationspecies = <FormArray>(
      this.addEventLocationForm.get("new_location_species")
    );
  ...
  checkEventLocationCommentMin() {
    // tslint:disable-next-line:max-line-length
    const siteDesciptionCommentLength =
      this.addEventLocationForm.get("site_description").value.length;
    const historyCommentLength =
      this.addEventLocationForm.get("history").value.length;
    // tslint:disable-next-line:max-line-length
    const envFactorsCommentLength = this.addEventLocationForm.get(
      "environmental_factors"
    ).value.length;
    const clinicalSignsCommentLength =
      this.addEventLocationForm.get("clinical_signs").value.length;
    // tslint:disable-next-line:max-line-length
    const generalCommentLength =
      this.addEventLocationForm.get("comment").value.length;
    if (
  ...
  viewContactDetailsDialog(locationContactIndex) {
    const contact_id = this.addEventLocationForm.get("new_location_contacts")
      .value[locationContactIndex].contact;
  ...
  openEditSpeciesDiagnosisDialog(
  ...
  openSpeciesDiagnosisRemoveConfirm(
    ...
    this.confirmDialogRef.afterClosed().subscribe((result) => {
      if (result === true) {
        ...
        // tslint:disable-next-line:max-line-length
        const diagnosisID = this.addEventLocationForm
          .get("new_location_species")
          ["controls"][locationSpeciesIndex].get("new_species_diagnoses")
  ...
  openAddSpeciesDiagnosisDialog(locationSpeciesIndex) {
    ...
    const speciesDiagnosisIndex =
      this.addSpeciesDiagnosis(locationSpeciesIndex);
    ...
    for (
      ...
      // tslint:disable-next-line:max-line-length
      const diagnosis = this.addEventLocationForm
        .get("new_location_species")
        ["controls"][locationSpeciesIndex].get("new_species_diagnoses")[
        "controls"
