  ...
  deleteCircle(id: number) {
    this.circleService.delete(id)
      .subscribe(
        ...
        error => {
          this.errorMessage = <any>error;
          this.openSnackBar('Error. Circle not deleted. Error message: ' + error, 'OK', 8000);
        }
      );
  ...
  masterToggle() {
    this.isAllSelected() ?
      (
        ...
        this.dataSource.circlesList.forEach(row => this.selection.deselect(row.id))
      ) :
      this.dataSource.circlesList.forEach(
        row => {
