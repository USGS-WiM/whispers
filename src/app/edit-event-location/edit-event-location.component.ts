...
export class EditEventLocationComponent implements OnInit {
  ...
  public filteredAdminLevelOnes: ReplaySubject<AdministrativeLevelOne[]> =
    new ReplaySubject<AdministrativeLevelOne[]>(1);
  adminLevelOneFilterCtrl: FormControl = new FormControl();

  public filteredAdminLevelTwos: ReplaySubject<AdministrativeLevelTwo[]> =
    new ReplaySubject<AdministrativeLevelTwo[]>(1);
  ...
  buildEditEventLocationForm() {
    this.editEventLocationForm = this.formBuilder.group(
      ...
      {
        validator: [this.endDateBeforeStart, this.endDateTodayorEarlier],
      }
    );
  }
  ...
  ngOnInit() {
    ...
    this.adminLevelOneService
      .queryAdminLevelOnes(this.data.eventLocationData.country)
      .subscribe(
        (adminLevelOnes) => {
    ...
    this.adminLevelTwoService
      .queryAdminLevelTwos(this.data.eventLocationData.administrative_level_one)
      .subscribe(
        (adminLevelTwos) => {
    ...
    this.editEventLocationForm.patchValue({
      ...
      start_date: APP_UTILITIES.timeZoneAdjust(
        this.data.eventLocationData.start_date
      ),
      end_date: APP_UTILITIES.timeZoneAdjust(
        this.data.eventLocationData.end_date
      ),
      country: this.data.eventLocationData.country.toString(),
      administrative_level_one:
        this.data.eventLocationData.administrative_level_one,
      administrative_level_two:
        this.data.eventLocationData.administrative_level_two,
    ...
    if (this.data.eventLocationData.land_ownership !== null) {
      this.editEventLocationForm
        .get("land_ownership")
        .setValue(this.data.eventLocationData.land_ownership.toString());
    }
  ...
  editCountyTooltip() {
    const string = FIELD_HELP_TEXT.editCountyTooltip;
    return string;
  }
  ...
  updateEventLocation(formValue) {
    ...
    this.eventLocationService.update(formValue).subscribe(
      (event) => {
        ...
        gtag("event", "click", {
          event_category: "Event Location Details",
          event_label: "Event Location Details Edited",
        });
      },
      (error) => {
        this.submitLoading = false;
        this.openSnackBar(
          "Error. Event location details not updated. Error message: " + error,
          "OK",
          8000
        );
      }
