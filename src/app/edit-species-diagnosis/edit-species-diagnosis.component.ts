...
export class EditSpeciesDiagnosisComponent implements OnInit {
  ...
  public filteredDiagnoses: ReplaySubject<Diagnosis[]> = new ReplaySubject<
    Diagnosis[]
  >(1);
  ...
  ngOnInit() {
    ...
    if (this.data.species_diagnosis_action === "add") {
      ...
      if (
        this.data.speciesdiagnosis.new_species_diagnosis_organizations.length >
        0
      ) {
        ...
        for (
          let i = 0,
            j =
              this.data.speciesdiagnosis.new_species_diagnosis_organizations
                .length;
          i < j;
          i++
        ) {
          ...
          this.speciesDiagnosisForm
            .get("new_species_diagnosis_organizations")
            ["controls"][i].get("org")
            .setValue(
              this.data.speciesdiagnosis.new_species_diagnosis_organizations[i]
            );
        }
      }
      ...
      this.speciesDiagnosisForm
        .get("diagnosis")
        .setValue(this.data.speciesdiagnosis.diagnosis);
      ...
      if (this.data.speciesdiagnosis.organizations.length > 0) {
        ...
        for (
          let i = 0, j = this.data.speciesdiagnosis.organizations.length;
          i < j;
          i++
        ) {
          this.addDiagnosisOrganization();
          ...
          this.speciesDiagnosisForm
            .get("new_species_diagnosis_organizations")
            ["controls"][i].get("org")
            .setValue(this.data.speciesdiagnosis.organizations[i]);
        }
      }
    }
    ...
    this.diagnosisService.getDiagnoses().subscribe(
      (diagnoses) => {
        ...
        if (this.data.locationspecies) {
          if (
            this.data.speciesdiagnosis !== undefined &&
            this.data.speciesdiagnosis.diagnosis !== null
          ) {
        ...
        this.diagnoses = this.diagnoses.filter(
          (diagnosis) =>
            diagnosis.id !==
            APP_SETTINGS.EVENT_INCOMPLETE_DIAGNOSIS_UNKNOWN.diagnosis
        );
    ...
    this.diagnosisBasisService.getDiagnosisBases().subscribe(
      (diagnosisBases) => {
        ...
        if (this.data.locationspecies) {
          if (
            this.data.speciesdiagnosis !== undefined &&
            this.data.speciesdiagnosis.basis !== null
          ) {
            // tslint:disable-next-line:max-line-length
            // the 'toString()' version of the below line used in past, may need to use conditionally
            ...
            this.speciesDiagnosisForm
              .get("basis")
              .setValue(this.data.speciesdiagnosis.basis);
          }
        }
      },
      (error) => {
    ...
    this.diagnosisCauseService.getDiagnosisCauses().subscribe(
      (diagnosisCauses) => {
        ...
        if (this.data.locationspecies) {
          if (
            this.data.speciesdiagnosis !== undefined &&
            this.data.speciesdiagnosis.cause !== null
          ) {
            // tslint:disable-next-line:max-line-length
            // the 'toString()' version of the below line used in past, may need to use conditionally
            ...
            this.speciesDiagnosisForm
              .get("cause")
              .setValue(this.data.speciesdiagnosis.cause);
          }
        }
      },
      (error) => {
  ...
  minimum(AC: AbstractControl) {
    const tested_count = AC.get("tested_count").value;
    const diagnosis_count = AC.get("diagnosis_count").value;
    if (tested_count < 1 && tested_count !== null) {
      this.speciesDiagnosisForm
        .get("tested_count")
        .setErrors({ minimum: true });
    }
    if (diagnosis_count < 1 && diagnosis_count !== null) {
      this.speciesDiagnosisForm
        .get("tested_count")
        .setErrors({ minimum: true });
    }
  ...
  addDiagnosisOrganization() {
    const control = <FormArray>(
      this.speciesDiagnosisForm.get("new_species_diagnosis_organizations")
    );
  ...
  private filterLaboratories(laboratoryIndex) {
    ...
    this.filteredLaboratories[laboratoryIndex].next(
      this.laboratories.filter(
        (laboratory) => laboratory.name.toLowerCase().indexOf(search) > -1
      )
    );
  }
  ...
  private filterDiagnosis() {
    ...
    this.filteredDiagnoses.next(
      this.diagnoses.filter(
        (diagnosis) => diagnosis.name.toLowerCase().indexOf(search) > -1
      )
    );
  }
  ...
  checkSuspectCompliance() {
    ...
    if (
      this.speciesDiagnosisForm["controls"].new_species_diagnosis_organizations[
        "controls"
      ].length === 0 &&
      !this.speciesDiagnosisForm.get("suspect").value
    ) {
      // if the diagnosis selected is NOT one of the unknowns, then show violation
      // tslint:disable-next-line:max-line-length
      if (
        this.speciesDiagnosisForm.get("diagnosis").value !==
  ...
  checkForDuplicateDiagnosis() {
    ...
    if (
      this.data.species_diagnosis_action === "add" ||
      this.data.species_diagnosis_action === "addToFormArray"
    ) {
      for (const existingDiagnosis of this.existingDiagnoses) {
        if (
          existingDiagnosis === this.speciesDiagnosisForm.get("diagnosis").value
        ) {
  ...
  speciesDiagnosisTooltip() {
    let string;
    if (
      this.data.species_diagnosis_action === "edit" ||
      this.data.species_diagnosis_action === "editToFormArray"
    ) {
  ...
  onSubmit(formValue) {
    ...
    formValue.new_species_diagnosis_organizations =
      new_species_diagnosis_orgs_array;
    ...
    if (this.data.species_diagnosis_action === "add") {
      ...
      this.locationSpeciesDiagnosisService.create(formValue).subscribe(
        (speciesdiagnosis) => {
          ...
          gtag("event", "click", {
            event_category: "Species Diagnosis",
            event_label:
              "Species Diagnosis Added, Diagnosis: " +
        ...
        (error) => {
          this.submitLoading = false;
          this.openSnackBar(
            "Error. Species Diagnosis not created. Error message: " + error,
            "OK",
            8000
          );
        }
      ...
      this.locationSpeciesDiagnosisService.update(formValue).subscribe(
        (contact) => {
          this.submitLoading = false;
          ...
          gtag("event", "click", {
            event_category: "Species Diagnosis",
            event_label:
              "Species Diagnosis Edited, Diagnosis: " + contact.diagnosis,
          });
        },
        (error) => {
          this.submitLoading = false;
