...
export class CircleService {
    ...
    return this.http
      ...
      .pipe(
        map((res: any) => {
          ...
          this.resultsCountService.updateCirclesResultsCount(res.count);
          return res.results;
        })
      );
  }
  ...
  public update(formValue): Observable<Event> {
    ...
    return this._http
      .put(APP_SETTINGS.CIRCLES_URL + formValue.id + "/", formValue, options)
      .pipe(
        map((response: Response) => <Event>response.json()),
        catchError(this.handleError)
      );
